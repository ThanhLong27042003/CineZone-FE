...
const AddShows = () => {
  ...
  const fetchData = async () => {
    try {
      // Fetch movies
      const moviesRes = await fetch("/api/admin/movies/getAllMovies/0/100");
      const moviesData = await moviesRes.json();
      setMovies(moviesData.result.content || []);
  ...
  const handleSubmit = async () => {
    if (!selectedMovie || !selectedRoom || !dateTimeInput || !showPrice) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.code === 1030 || data.code === 1014) {
          toast.error("Show time overlaps with existing show in this room!", {
            duration: 4000,
          });
        } else {
          toast.error(data.message || "Failed to create show");
        }
      }
    } catch (error) {
            ...
            {rooms.map((room) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  <FaDoorOpen className="text-2xl text-blue-500" />
                  <div>
        ...
        >
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              onChange={(e) => setShowPrice(e.target.value)}
              placeholder="Enter show price"
              className="w-full pl-12 pr-4 py-4 rounded-xl border-2 border-gray-300
      ...
      {showPrice && (
        ...
        >
          ...
          <input
            ...
            value={dateTimeInput}
            onChange={(e) => setDateTimeInput(e.target.value)}
            className="w-full px-4 py-4 rounded-xl border-2 border-gray-300
                     focus:border-purple-500 outline-none"
          />
          ...
          <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
            <p className="text-sm text-yellow-800 dark:text-yellow-200">
              ⚠️ Make sure this time doesn't overlap with existing shows in the
              selected room
            </p>
          </div>
        </motion.div>
      ...
      {dateTimeInput && (
        <motion.button
          ...
          className="w-full py-4 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500
                   text-white font-bold text-lg shadow-2xl disabled:opacity-50
                   flex items-center gap-3 justify-center"
        >
          <FaCheck />
          {loading ? "Creating..." : "Add Show"}
        </motion.button>
      )}
    </div>
  );
